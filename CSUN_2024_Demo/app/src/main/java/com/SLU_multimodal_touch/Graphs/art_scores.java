...
public class art_scores extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.art_scores);
        ...
        if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
            // In landscape
            mv.setImageResource(R.drawable.art_scores);
        } else {
            // In portrait
            mv.setImageResource(R.drawable.art_scores);
        }
...
//                grtv.setText("You swiped up with " + fingers + " finger(s).");
                if (fingers == 1) {
                    ...
                    Intent back_to_graph_selector = new Intent(art_scores.this, GraphSelector.class);
            ...
            public void run() {
                ...
                if (sfg.finger_count == 1){
                    ...
                    if (f0_index != -1) {
                        ...
                        // setting text to speech for art_scores
                        if (X_screenPercent > 0.36 && X_screenPercent < 0.63 && Y_screenPercent < 0.07 && Y_screenPercent > 0.02){
                            if (!hasSpoken) {
                                tts.speak("Title: Scores in Two Subjects! A school sought to assess the performance of three students in two subjects by recording their scores. The two bar charts presented below provide a comparative analysis of the students' performance in Graphic Design and Art scores. The initial chart illustrates the Graphic Design scores, while the subsequent chart displays the Art scores for the same three students. Refer to the graphs for answers to the questions.", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        ...
                        } else if (X_screenPercent > 0.48 && X_screenPercent < 0.58 && Y_screenPercent < 0.97 && Y_screenPercent > 0.94){
                            if (!hasSpoken) {
                                tts.speak("X-axis title: Subjects", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.17 && X_screenPercent < 0.41 && Y_screenPercent < 0.93 && Y_screenPercent > 0.91){
                            if (!hasSpoken) {
                                tts.speak("Graphic Design Scores", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.71 && X_screenPercent < 0.82 && Y_screenPercent < 0.93 && Y_screenPercent > 0.91){
                            if (!hasSpoken) {
                                tts.speak("Art Scores", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.04 && X_screenPercent < 0.98 && Y_screenPercent < 0.88 && Y_screenPercent > 0.85){
                            if (!hasSpoken) {
                                tts.speak("0", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.04 && X_screenPercent < 0.98 && Y_screenPercent < 0.81 && Y_screenPercent > 0.77){
                            if (!hasSpoken) {
                                tts.speak("5", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.04 && X_screenPercent < 0.98 && Y_screenPercent < 0.73 && Y_screenPercent > 0.70){
                            if (!hasSpoken) {
                                tts.speak("10", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.04 && X_screenPercent < 0.98 && Y_screenPercent < 0.65 && Y_screenPercent > 0.62){
                            if (!hasSpoken) {
                                tts.speak("15", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.04 && X_screenPercent < 0.98 && Y_screenPercent < 0.58 && Y_screenPercent > 0.54){
                            if (!hasSpoken) {
                        ...
                        } else if (X_screenPercent > 0.04 && X_screenPercent < 0.98 && Y_screenPercent < 0.42 && Y_screenPercent > 0.39){
                            if (!hasSpoken) {
                                tts.speak("30", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.04 && X_screenPercent < 0.98 && Y_screenPercent < 0.35 && Y_screenPercent > 0.31){
                            if (!hasSpoken) {
                                tts.speak("35", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.04 && X_screenPercent < 0.98 && Y_screenPercent < 0.27 && Y_screenPercent > 0.24){
                            if (!hasSpoken) {
                                tts.speak("40", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.04 && X_screenPercent < 0.98 && Y_screenPercent < 0.19 && Y_screenPercent > 0.17){
                            if (!hasSpoken) {
    ...
    public void logger(String file_name) throws IOException {
        ...
        final Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                if (ContextCompat.checkSelfPermission(art_scores.this,Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions(art_scores.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE},1);
                }
                else {
