...
public class fruit_types extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.fruit_types);
        ...
        if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
            // In landscape
            mv.setImageResource(R.drawable.fruit_types);
        } else {
            // In portrait
            mv.setImageResource(R.drawable.fruit_types);
        }
...
//                grtv.setText("You swiped up with " + fingers + " finger(s).");
                if (fingers == 1) {
                    ...
                    Intent back_to_graph_selector = new Intent(fruit_types.this, GraphSelector.class);
            ...
            public void run() {
                ...
                if (sfg.finger_count == 1){
                    ...
                    if (f0_index != -1) {
                        ...
                        // setting text to speech for fruit_types
                        if (X_screenPercent > 0.33 && X_screenPercent < 0.66 && Y_screenPercent < 0.06 && Y_screenPercent > 0.02){
                            if (!hasSpoken) {
                                tts.speak("Title: Preference for Types of Fruits! School scouts aimed to distribute fruits to children and documented students' preferences for three types of fruits. The provided bar graph illustrates the preferences for banana, mango, and strawberry among boys and girls in the 5th grade. Refer to the graph below to respond to the questions.", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.01 && X_screenPercent < 0.03 && Y_screenPercent < 0.66 && Y_screenPercent > 0.31){
                            if (!hasSpoken) {
                                tts.speak("Y-axis title: Number of Children", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.47 && X_screenPercent < 0.58 && Y_screenPercent < 0.97 && Y_screenPercent > 0.93){
                            if (!hasSpoken) {
                                tts.speak("X-axis title: Fruit Types", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.13 && X_screenPercent < 0.33 && Y_screenPercent < 0.92 && Y_screenPercent > 0.89){
                            if (!hasSpoken) {
                                tts.speak("Banana", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.43 && X_screenPercent < 0.63 && Y_screenPercent < 0.92 && Y_screenPercent > 0.89){
                            if (!hasSpoken) {
                                tts.speak("Mango", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
                        } else if (X_screenPercent > 0.73 && X_screenPercent < 0.93 && Y_screenPercent < 0.92 && Y_screenPercent > 0.89){
                            if (!hasSpoken) {
                                tts.speak("Strawberry", TextToSpeech.QUEUE_FLUSH, null);
                                hasSpoken = true;
                            }
    ...
    public void logger(String file_name) throws IOException {
        ...
        final Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                if (ContextCompat.checkSelfPermission(fruit_types.this,Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions(fruit_types.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE},1);
                }
                else {
