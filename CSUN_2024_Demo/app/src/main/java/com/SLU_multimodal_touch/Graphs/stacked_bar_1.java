...
public class stacked_bar_1 extends AppCompatActivity {
    ...
    Boolean empty_spoken = false;
    Boolean inside_spoken = false;
    Boolean line_spoken = false;
    Boolean vertex_spoken = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.stacked_bar_1);
        ...
        if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
            // In landscape
            mv.setImageResource(R.drawable.stacked_bar_1);
        } else {
            // In portrait
            mv.setImageResource(R.drawable.stacked_bar_1);
        }
...
//                    overridePendingTransition(R.anim.slide_in_down, R.anim.slide_out_down);
            ...
            public boolean onSwipeLeft(int fingers, long gestureDuration, double gestureDistance) {
                ...
                if (fingers == 1) {
                    ...
                    // Create an Intent that takes us from this activity to another
//                    Intent go_to_next_screen = new Intent(stacked_bar_1.this, screen_1.class);
                    // Activate the intent to go to that screen
//                    startActivity(go_to_next_screen);
...
//                    tts.speak("There is a certain number of small regions on screen in different locations. They all produce a different vibration pattern. Can you identify how many regions are on screen?", TextToSpeech.QUEUE_FLUSH, null);
                    ...
                    Intent back_to_graph_selector = new Intent(stacked_bar_1.this, GraphSelector.class);
    ...
    public void monitor(ImageView mv, TextView coord_view, TextView output_view) {
        ...
        vertex_sound = MediaPlayer.create(this, R.raw.beep_continuous_2);
        AssetFileDescriptor afd_vertex_sound = getResources().openRawResourceFd(R.raw.beep_continuous_2);
        FileDescriptor fd_vertex_sound = afd_inside_sound.getFileDescriptor();

        // Set the MediaPlayer objects to loop
        empty_sound.setLooping(true);
        inside_sound.setLooping(true);
        line_sound.setLooping(true);
        vertex_sound.setLooping(true);
        ...
        handler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (sfg.finger_count == 0){
                    ...
                    if (vertex_sound.isPlaying()) {
                        vertex_sound.stop();
                    }

                    // Stop any TTS speakers and release the resources
//                    tts.shutdown();
                }
                ...
                if (sfg.finger_count >= 1){
                    ...
                    if (f0_index != -1) {
                        ...
                        // If finger is ON A VERTEX
                            ...
                            // TTS Speech + Sound playback
                            if (!vertex_spoken) {
                                tts.speak("Light Blue", TextToSpeech.QUEUE_FLUSH, null);
                                vertex_spoken = true;
                            }
                            else if (!tts.isSpeaking()) {
                                // Play the inside sound
                                if (vertex_sound.getDuration() > 0 ) {
                                    // The MediaPlayer has been initialized. Give it your commands.

                                    // Modify the SPEAKER VOLUME according to the X location proportion
                                    vertex_sound.setVolume((1 - audio_x), audio_x);
                                    ...
                                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                                        PlaybackParams params = new PlaybackParams();
                                        params.setPitch(pitch);
                                        vertex_sound.setPlaybackParams(params);
                                    }
                                }
                                else {
                                    // The media player has NOT been initialized, because getDuration() returns -1. Initialize it!
                                    ...
                                    try {
                                        // Get the path to the desired sound
                                        vertex_sound.setDataSource(fd_vertex_sound, afd_vertex_sound.getStartOffset(), afd_vertex_sound.getLength());
                                        vertex_sound.prepare();
                                    } catch (IOException e) {
                                        throw new RuntimeException(e);
                                    }
                                }
                        ...
                        // If finger is on EMPTY space
                            ...
                            // TTS Speech + Sound playback
                            if (!empty_spoken) {
                                tts.speak("Empty Space", TextToSpeech.QUEUE_FLUSH, null);
                                empty_spoken = true;
                            }
                            else if (!tts.isSpeaking()) {
                                // Play the inside sound
    ...
    public void logger(String file_name) throws IOException {
        ...
        final Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                if (ContextCompat.checkSelfPermission(stacked_bar_1.this,Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions(stacked_bar_1.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE},1);
                }
                else {
